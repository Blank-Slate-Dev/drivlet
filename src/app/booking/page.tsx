...
export default function BookingPage() {
  ...
  const distanceZoneInfo: ZoneInfo | null = useMemo(() => {
    if (selectedPlaceDetails?.lat != null && selectedPlaceDetails?.lng != null && garageLat != null && garageLng != null) {
  ...
  const formatDateDisplay = (s: string) => { if (!s) return ''; return new Date(s).toLocaleDateString('en-AU', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }); };
  const getTimeLabel = (value: string, options: TimeOption[]): string => options.find(t => t.value === value)?.label || value;

  const goToStep = useCallback((step: Step) => { setSubmitError(null); setCurrentStep(step); window.scrollTo({ top: 0, behavior: 'smooth' }); }, []);
  ...
  const handlePaymentSuccess = async () => {
    if (paymentIntentId) {
      try {
        const response = await fetch('/api/bookings/create-after-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ paymentIntentId }) });
        const data = await response.json();
  ...
  if (authStatus === 'loading') {
    return (<main className="min-h-screen bg-gradient-to-br from-emerald-800 via-emerald-700 to-teal-700 flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-white" /></main>);
  }
              ...
              <div className="hidden sm:flex flex-1 justify-center">
                <div className="flex items-center">
                  {STEPS.map((step, index) => {
                    const isActive = currentStep === step.key; const isComplete = stepIndex > index; const StepIcon = step.icon;
                    return (
                      <div key={step.key} className="flex items-center">
                        {index > 0 && (<div className="w-14 h-0.5 rounded-full bg-white/15 overflow-hidden mx-1"><div className="h-full bg-emerald-400 rounded-full transition-all duration-500 ease-out" style={{ width: isComplete || isActive ? '100%' : '0%' }} /></div>)}
                        <div className="flex flex-col items-center">
                          <div className={`flex h-8 w-8 items-center justify-center rounded-full transition-all duration-300 ${isActive ? 'bg-white text-emerald-700 shadow-lg shadow-emerald-500/20' : isComplete ? 'bg-emerald-400 text-white' : 'bg-white/10 text-white/30'}`}>{isComplete ? <Check className="h-4 w-4" /> : <StepIcon className="h-4 w-4" />}</div>
                          <span className={`mt-1 text-[10px] font-medium leading-none transition-colors duration-300 ${isActive ? 'text-white' : isComplete ? 'text-emerald-300' : 'text-white/25'}`}>{step.label}</span>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="flex sm:hidden justify-center mt-3">
              <div className="flex items-center">
                {STEPS.map((step, index) => {
                  ...
                  return (
                    <div key={step.key} className="flex items-center">
                      {index > 0 && (<div className="w-8 h-0.5 rounded-full bg-white/15 overflow-hidden mx-1"><div className="h-full bg-emerald-400 rounded-full transition-all duration-500 ease-out" style={{ width: isComplete || isActive ? '100%' : '0%' }} /></div>)}
                      <div className="flex flex-col items-center">
                        <div className={`flex h-7 w-7 items-center justify-center rounded-full transition-all duration-300 ${isActive ? 'bg-white text-emerald-700 shadow-lg shadow-emerald-500/20' : isComplete ? 'bg-emerald-400 text-white' : 'bg-white/10 text-white/30'}`}>{isComplete ? <Check className="h-3.5 w-3.5" /> : <StepIcon className="h-3.5 w-3.5" />}</div>
                        <span className={`mt-1 text-[9px] font-medium leading-none transition-colors duration-300 ${isActive ? 'text-white' : isComplete ? 'text-emerald-300' : 'text-white/25'}`}>{step.label}</span>
                      </div>
                    </div>
                  );
                })}
      ...
      <div className="relative z-10">
        <div className="mx-auto max-w-2xl px-4 py-8 sm:py-10 pb-12">
          <AnimatePresence mode="wait">
            <motion.div key={currentStep} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>{renderStep()}</motion.div>
          </AnimatePresence>
        </div>
      </div>
    </main>
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-emerald-100 text-sm sm:text-base">We&apos;ll collect your car, take it to the garage, and bring it back</p>
        </div>
        <div className="rounded-3xl border border-white/20 bg-white/95 backdrop-blur-sm shadow-2xl p-6 sm:p-8 space-y-6">
          ...
          {!isAuthenticated ? (
            <div className="space-y-4">
              ...
              <p className="text-xs text-slate-500">Have an account?{' '}<Link href="/login" className="font-medium text-emerald-600 hover:underline">Sign in</Link></p>
            </div>
          ) : (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-600 font-semibold text-white">{(session?.user?.username || session?.user?.email || '?').charAt(0).toUpperCase()}</div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Pick-up Address *</label>
            <AddressAutocomplete
              value={pickupAddress}
  ...
  function renderVehicle() {
    return (
      <div className="space-y-6">
        <div className="text-center"><h1 className="text-2xl sm:text-3xl font-bold text-white">Vehicle Details</h1><p className="mt-2 text-emerald-100 text-sm sm:text-base">Help our drivers identify your car</p></div>
        <div className="rounded-3xl border border-white/20 bg-white/95 backdrop-blur-sm shadow-2xl p-6 sm:p-8 space-y-6">
          {renderErrorBanner()}
          <div className={`rounded-xl border p-4 transition-colors ${isHighValueVehicle ? 'bg-red-50 border-red-200' : 'bg-slate-50 border-slate-200'}`}>
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={isHighValueVehicle} onChange={(e) => setIsHighValueVehicle(e.target.checked)} className={`mt-1 h-5 w-5 rounded border-2 cursor-pointer ${isHighValueVehicle ? 'border-red-500 accent-red-500' : 'border-slate-300 accent-emerald-600'}`} />
              <div>
                <span className={`text-sm font-medium ${isHighValueVehicle ? 'text-red-700' : 'text-slate-700'}`}>My vehicle is valued at $100,000 or more</span>
                {isHighValueVehicle && (<p className="mt-2 text-sm text-red-600">For insurance reasons, we currently cannot service vehicles valued at $100,000+. We&apos;re working to expand coverage.</p>)}
              </div>
            </label>
          </div>
          <div className={`space-y-6 transition-opacity ${isHighValueVehicle ? 'opacity-40 pointer-events-none' : ''}`}>
            ...
            {distanceZoneInfo && selectedPlaceDetails?.lat != null && garageLat != null && garageLng != null && (
              <div className={`rounded-xl border-2 p-4 space-y-3 ${distanceZoneInfo.zone === 'green' ? 'border-emerald-300 bg-emerald-50/50' : distanceZoneInfo.zone === 'yellow' ? 'border-yellow-300 bg-yellow-50/50' : distanceZoneInfo.zone === 'orange' ? 'border-orange-300 bg-orange-50/50' : 'border-red-300 bg-red-50/50'}`}>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-bold ${ZONE_BADGE_STYLES[distanceZoneInfo.zone]}`}><span className={`h-2.5 w-2.5 rounded-full ${distanceZoneInfo.zone === 'green' ? 'bg-emerald-500' : distanceZoneInfo.zone === 'yellow' ? 'bg-yellow-500' : distanceZoneInfo.zone === 'orange' ? 'bg-orange-500' : 'bg-red-500'}`} />{distanceZoneInfo.label}</span>
                ...
                {isRedZone && (<div className="rounded-xl border border-red-300 bg-red-100 p-3"><p className="text-sm text-red-700"><span className="font-semibold">Out of range.</span> Please choose a closer garage or{' '}<Link href="/contact" className="font-semibold underline">contact us</Link>.</p></div>)}
            ...
            {!distanceZoneInfo && pickupAddress.trim() && garageSearch.trim() && (<div className="flex items-start gap-2 rounded-xl bg-blue-50 border border-blue-200 p-3"><Info className="h-4 w-4 flex-shrink-0 text-blue-500 mt-0.5" /><p className="text-xs text-blue-700">Select your addresses from the dropdown suggestions to see the distance zone.</p></div>)}
          </div>
  ...
  function renderSchedule() {
    return (
      <div className="space-y-6">
        <div className="text-center"><h1 className="text-2xl sm:text-3xl font-bold text-white">Schedule & Service</h1><p className="mt-2 text-emerald-100 text-sm sm:text-base">Choose your date, time slots, and service type</p></div>
        <div className="rounded-3xl border border-white/20 bg-white/95 backdrop-blur-sm shadow-2xl p-6 sm:p-8 space-y-6">
          {renderErrorBanner()}
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700">Service Type *</label>
            <div className="grid gap-3">
              {SERVICE_TYPES.map((svc) => { const SvcIcon = getServiceIcon(svc.value); return (
                <button key={svc.value} type="button" onClick={() => setSelectedServiceType(svc.value)} className={`relative rounded-xl border-2 p-4 text-left transition-all ${selectedServiceType === svc.value ? 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-500/20' : 'border-slate-200 bg-white hover:border-emerald-300'}`}>
                  <div className="flex items-start gap-3"><SvcIcon className={`h-5 w-5 mt-0.5 ${selectedServiceType === svc.value ? 'text-emerald-600' : 'text-slate-400'}`} /><div className="flex-1"><div className="flex items-center justify-between"><h4 className="font-semibold text-slate-900">{svc.label}</h4><span className="text-sm text-slate-500">~{svc.estimatedHours}hrs</span></div><p className="mt-1 text-sm text-slate-600">{svc.description}</p></div>{selectedServiceType === svc.value && <Check className="h-5 w-5 text-emerald-600 mt-0.5" />}</div>
                </button>); })}
            </div>
          </div>
          <div>
          ...
          {FEATURES.SERVICE_SELECTION && (<ServiceSelector selectedServices={selectedServices} onServicesChange={setSelectedServices} primaryCategory={primaryServiceCategory} onPrimaryCategoryChange={setPrimaryServiceCategory} serviceNotes={serviceNotes} onServiceNotesChange={setServiceNotes} disabled={isProcessing} />)}
          <div className="flex items-center justify-between pt-2">
    ...
    function SlotPicker({ label, slots, selected, onSelect }: { label: string; slots: { slot: string; label: string; available: number; isFull: boolean }[]; selected: string; onSelect: (v: string) => void }) {
      return (<div className="space-y-2"><label className="text-sm font-medium text-slate-700">{label}</label>{!serviceDate ? (<p className="text-sm text-slate-400 py-3">Select a date first</p>) : loadingSlots ? (<div className="flex justify-center py-6"><Loader2 className="h-6 w-6 animate-spin text-emerald-500" /></div>) : (<div className="grid gap-2">{slots.map((slot) => (<button key={slot.slot} type="button" onClick={() => !slot.isFull && onSelect(slot.slot)} disabled={slot.isFull} className={`rounded-xl border-2 px-4 py-3 text-left transition-all ${slot.isFull ? 'opacity-50 border-slate-200 bg-slate-50 cursor-not-allowed' : selected === slot.slot ? 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-500/20' : 'border-slate-200 bg-white hover:border-emerald-300'}`}><div className="flex items-center justify-between"><div className="flex items-center gap-3"><Clock className="h-4 w-4 text-slate-400" /><span className="font-medium text-slate-900">{slot.label}</span></div><div className="flex items-center gap-2">{slot.isFull ? <span className="rounded-full bg-red-100 px-3 py-1 text-xs font-medium text-red-700">Full</span> : <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-700">{slot.available} left</span>}{selected === slot.slot && <Check className="h-5 w-5 text-emerald-600" />}</div></div></button>))}</div>)}</div>);
    }
  }
  ...
  function renderReview() {
    ...
    return (
      <div className="space-y-6">
        <div className="text-center"><h1 className="text-2xl sm:text-3xl font-bold text-white">Review Your Booking</h1><p className="mt-2 text-emerald-100 text-sm sm:text-base">Confirm everything looks right</p></div>
        <div className="rounded-3xl border border-white/20 bg-white/95 backdrop-blur-sm shadow-2xl p-6 sm:p-8 space-y-4">
          {renderErrorBanner()}
          <ReviewCard icon={<User className="h-4 w-4 text-emerald-600" />} title="Customer" editStep="details"><p className="font-medium text-slate-900">{customerName}</p><p className="text-slate-600">{customerEmail}</p>{customerPhone && <p className="text-slate-600">{customerPhone}</p>}</ReviewCard>
          <ReviewCard icon={<Car className="h-4 w-4 text-emerald-600" />} title="Vehicle" editStep="vehicle"><div className="flex items-center justify-between"><span className="font-medium text-slate-900">{vehicleYear} {vehicleModel}</span><div className="transform scale-75 origin-right"><RegistrationPlate plate={regoPlate} state={regoState} /></div></div><div className="mt-2 flex justify-between border-t border-slate-200 pt-2"><span className="text-slate-500">Transmission</span><span className={`font-medium ${transmissionType === 'manual' ? 'text-amber-600' : 'text-slate-900'}`}>{transmissionType === 'manual' ? 'Manual' : 'Automatic'}</span></div></ReviewCard>
          <ReviewCard icon={<Calendar className="h-4 w-4 text-emerald-600" />} title="Schedule" editStep="schedule"><p className="font-medium text-slate-900">{formatDateDisplay(serviceDate)}</p><div className="mt-2 grid grid-cols-2 gap-2"><div><span className="text-slate-500 text-xs">Pickup</span><p className="font-medium text-slate-900">{getPickupSlotLabel(selectedPickupSlot)}</p></div><div><span className="text-slate-500 text-xs">Drop-off</span><p className="font-medium text-slate-900">{getDropoffSlotLabel(selectedDropoffSlot)}</p></div></div><div className="mt-2 border-t border-slate-200 pt-2"><span className="text-slate-500 text-xs">Service type</span><p className="font-medium text-slate-900">{svcInfo?.label} (~{svcInfo?.estimatedHours}hrs)</p></div></ReviewCard>
  ...
  function renderPayment() {
    ...
    return (
      <div className="space-y-6">
        <div className="text-center"><h1 className="text-2xl sm:text-3xl font-bold text-white">Secure Payment</h1><p className="mt-2 text-emerald-100 text-sm sm:text-base">Complete your booking</p></div>
      ...
      <div className="flex min-h-[60vh] items-center justify-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md">
          <div className="rounded-3xl border border-white/20 bg-white/95 backdrop-blur-sm p-8 shadow-2xl text-center">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', delay: 0.2 }} className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-emerald-100"><CheckCircle2 className="h-10 w-10 text-emerald-600" /></motion.div>
  ...
  function renderErrorBanner() {
    ...
    return (<div className="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 p-4"><AlertCircle className="h-5 w-5 flex-shrink-0 text-red-500" /><p className="text-sm text-red-800">{submitError}</p></div>);
  }
  ...
  function ReviewCard({ icon, title, editStep, children }: { icon: React.ReactNode; title: string; editStep: Step; children: React.ReactNode }) {
    return (<div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between mb-3"><h3 className="flex items-center gap-2 text-sm font-semibold text-slate-700">{icon} {title}</h3><button type="button" onClick={() => goToStep(editStep)} className="text-xs text-emerald-600 hover:underline font-medium">Edit</button></div><div className="text-sm space-y-1">{children}</div></div>);
  }
}
